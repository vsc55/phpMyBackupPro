            ...
            if (is_array($remote_files)) {
                ...
                if (is_array($delete_files)) {
                    foreach ($delete_files as $filename) {
                        ...
                        if (!$check) {
                            $out .= PMBP_addOutput(sprintf(F_FTP_5, $dest_file), "red");
                        } else {
                            $out .= PMBP_addOutput(sprintf(F_FTP_6, $dest_file), "green");
                        }
...
function PMBP_get_db_list()
{
    ...
    if ($con = PMBP_mysql_connect()) {
        ...
        $db_list = @mysqli_query($con, $query);
        while ($row = @mysqli_fetch_array($db_list)) {
            if (@mysqli_select_db($con, $row['Database'])) {
                $list[] = $row['Database'];
            }
        }
    }
    ...
    switch ($mode) {
        ...
        case "comp":
            if (PMBP_file_info("gzip", $path)) {
                return "gzip";
            } elseif (PMBP_file_info("zip", $path)) {
                return "zip";
            } else {
                return F_NO;
            }
...
function PMBP_addOutput($string, $class)
{
    ...
    if ($mode == "web") {
        $out .= "<div class=\"" . $class . "\">" . $string . "</div>\n";
    } else if ($mode == "shell") {
        $out .= $string . "\n";
    } else if ($mode == "incl") {
        $out .= $string . "<br>\n";
...
function PMBP_save_FTP_and_Email($files, $packed = false)
{
    ...
    if ($packed) {
        ...
        // close the FTP connection
        if (!$ftp_connect_failed && @function_exists("ftp_close")) {
            @ftp_close($conn_id);
        }
